...
async function deriveKey(passphrase: string, salt: Uint8Array | ArrayBuffer): Promise<CryptoKey> {
  ...
  const saltAB = saltU8.buffer.slice(saltU8.byteOffset, saltU8.byteOffset + saltU8.byteLength) as ArrayBuffer;
...
export async function encryptPrivateKeyPkcs8B64(pkcs8b64: string, passphrase: string): Promise<VaultBlob> {
...
export async function decryptPrivateKeyPkcs8B64(vault: VaultBlob, passphrase: string): Promise<string> {
  const saltU8 = b64decode(vault.salt_b64);
  const salt = saltU8.buffer.slice(saltU8.byteOffset, saltU8.byteOffset + saltU8.byteLength) as ArrayBuffer;
