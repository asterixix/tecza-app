...
export function useMessages(conversationId?: string) {
  ...
  const { data: conv, error } = await supabase!
      ...
      if (conv.type === "direct") {
        ...
        const profilesTyped: Array<{ id: string; username: string; display_name: string; avatar_url?: string }> | undefined = (conv as unknown as { profiles?: Array<{ id: string; username: string; display_name: string; avatar_url?: string }> }).profiles;
  ...
  const { data, error } = await supabase!
      ...
      const decryptedMessages = await Promise.all(
        data.map(async (msg) => {
          ...
          if (msg.media_path) {
            const bucket = msg.type === "image" ? "message-images" : msg.type === "video" ? "message-videos" : "message-files";
  ...
  const messageData: Record<string, unknown> = {
      ...
      if (file) {
        ...
        const encrypted = await MessageEncryption.encryptFile(arrayBuffer, conversationKeyRef.current);
        
        // Upload to storage
        const fileName = `${user.id}/${Date.now()}_${file.name}`;
        const bucket = type === "image" ? "message-images" : 
                      type === "video" ? "message-videos" : "message-files";
  ...
  const requestSecureDelete = useCallback(async (messageId: string) => {
    try {
