...
export function useAuth(): AuthState {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!isMounted) return;
      setState({ user: data.user ? { id: data.user.id, email: data.user.email } : null, loading: false });
    })();

    const { data: sub } = supabase!.auth.onAuthStateChange(async (_event, session) => {
      setState((s) => ({ ...s, user: session?.user ? { id: session.user.id, email: session.user.email } : null }));
      if (session?.user) {
