...
export default function RolesAdmin() {
  ...
  const [list, setList] = useState<Array<{id:string; username:string|null; roles:string[]; badges:string[] }>>([])

  const refresh = useCallback(async () => {
    if (!supabase) return
  const { data } = await supabase.from('profiles').select('id,username,roles,badges').order('username')
  const typed = (data as Array<{ id: string; username: string | null; roles: string[] | null; badges: string[] | null }> | null) || []
  setList(typed.map(r => ({ id: r.id, username: r.username, roles: r.roles || [], badges: r.badges || [] })))
  }, [supabase])
  ...
  async function toggleRole(userId: string, role: string) {
    if (!supabase) return
    const row = list.find(x => x.id === userId); if (!row) return
  ...
  <h1 className="text-2xl font-semibold mb-4">Role użytkowników</h1>
      <div className="mb-4">
        <input className="w-full max-w-sm rounded-md border px-3 py-2" placeholder="Szukaj użytkownika" value={query} onChange={e => setQuery(e.target.value)} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted">
            <tr>
              ...
              {ALL_ROLES.map(r => (<th key={r} className="text-left p-2 whitespace-nowrap">{r}</th>))}
            </tr>
          </thead>
          <tbody>
            {list.filter(x => !query || (x.username||"").toLowerCase().includes(query.toLowerCase())).map(u => (
              <tr key={u.id} className="border-t">
                <td className="p-2 font-medium">{u.username || u.id.slice(0,8)}{u.username?"":"…"}</td>
                {ALL_ROLES.map(r => {
                  const checked = (u.roles||[]).includes(r)
                  return (
                    <td key={r} className="p-2">
                      <label className="inline-flex items-center gap-2">
                        <input type="checkbox" checked={checked} onChange={() => toggleRole(u.id, r)} />
      ...
      <div className="overflow-x-auto rounded-md border">
        <table className="w-full text-sm">
          <thead className="bg-muted">
            <tr>
              <th className="text-left p-2">Użytkownik</th>
              {ALL_BADGES.map(b => (<th key={b} className="text-left p-2 whitespace-nowrap">{b}</th>))}
            </tr>
          </thead>
          <tbody>
            {list.filter(x => !query || (x.username||"").toLowerCase().includes(query.toLowerCase())).map(u => (
                ...
                {ALL_BADGES.map(b => {
                  ...
                  return (
                    <td key={b} className="p-2">
                      <label className="inline-flex items-center gap-2">
                        <input type="checkbox" checked={checked} onChange={() => toggleBadge(u.id, b)} />
