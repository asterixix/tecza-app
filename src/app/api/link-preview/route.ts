...
export async function GET(req: Request) {
  try {
    ...
    // Walidacja URL
    let parsed: URL
    try { parsed = new URL(url) } catch { return new Response(JSON.stringify({ error: 'Invalid url' }), { status: 400 }) }

    const ac = new AbortController()
    const t = setTimeout(() => ac.abort(), 7000)
    const res = await fetch(parsed.toString(), { signal: ac.signal, headers: { 'User-Agent': 'tecza-app-linkpreview/1.0' } })
    clearTimeout(t)
    if (!res.ok) return new Response(JSON.stringify({ error: 'Fetch failed', status: res.status }), { status: 502 })
