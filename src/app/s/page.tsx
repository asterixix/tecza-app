          ...
          <Button variant="outline" size="sm" onClick={async () => {
            ...
            <Card>
              <CardContent className="p-4 grid gap-3">
                ...
                <p className="text-sm text-muted-foreground">Zabezpiecz swój klucz prywatny hasłem i przechowuj zaszyfrowaną kopię w profilu. Dzięki temu możesz odszyfrowywać wiadomości na innych urządzeniach.</p>
                <div className="grid md:grid-cols-2 gap-2 max-w-xl">
                  <Input type="password" placeholder="Hasło do sejfu" value={password} onChange={(e) => setPassword(e.target.value)} />
                  <div className="flex gap-2">
                    <Button size="sm" onClick={async () => {
                      try {
                        ...
                        if (!pkcs8b64) { toast.error("Brak klucza prywatnego w pamięci. Zaloguj się ponownie."); return }
                        if (!password) { toast.error("Podaj hasło do sejfu"); return }
                        const vault = await encryptPrivateKeyPkcs8B64(pkcs8b64, password)
                        const { error } = await supabase.from("profiles").update({ private_key_vault: vault as unknown as VaultBlob }).eq("id", (await supabase.auth.getUser()).data.user?.id)
                        if (error) throw error
                    ...
                    <Button size="sm" variant="outline" onClick={async () => {
                      try {
                        if (!supabase) { toast.error("Brak konfiguracji Supabase"); return }
                        const { data } = await supabase.from("profiles").select("private_key_vault").eq("id", (await supabase.auth.getUser()).data.user?.id).maybeSingle()
                        const vault = (data?.private_key_vault || null) as VaultBlob | null
                        if (!vault) { toast.error("Brak sejfu w profilu"); return }
                        if (!password) { toast.error("Podaj hasło do sejfu"); return }
                    ...
                    }}>Odszyfruj na tym urządzeniu</Button>
                  </div>
                </div>
                <div className="text-xs text-muted-foreground">Uwaga: hasło nie jest przechowywane. Zadbaj o jego zapamiętanie.</div>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
