...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    // Detect recovery from hash and Supabase auth events
  ...
  async function reset() {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      // Kierujemy bezpośrednio na /l, aby nie utracić fragmentu URL (#type=recovery)
      redirectTo:
        typeof window !== "undefined"
  ...
  // Ustawienie nowego hasła po wejściu z linku recovery
  async function submitNewPassword(e: React.FormEvent) {
    e.preventDefault()
    if (!supabase) return toast.error("Brak konfiguracji Supabase")
    if (newPassword.length < 8)
      return toast.error("Hasło musi mieć min. 8 znaków")
    ...
    try {
      ...
      setIsRecovery(false)
      // Po zmianie hasła Supabase utrzymuje sesję z linku recovery
      // route() w onAuthStateChange przeniesie użytkownika dalej
    } catch (err: unknown) {
      const msg =
  ...
  return (
    <div className="mx-auto max-w-md px-4 md:px-6 py-10 md:py-14">
      ...
      <div className="mb-4">
        <Badge variant="secondary">Logowanie i rejestracja</Badge>
      </div>
      <Card>
        <CardContent className="p-6">
          {!isRecovery ? (
            <>
              ...
              <p className="mt-1 text-muted-foreground">
                Wprowadź swój email i hasło, lub zaloguj się korzystając z konta
                Google lub Discorda.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="mt-1 text-muted-foreground">
                Wprowadź nowe hasło do swojego konta. Po zapisaniu nastąpi
            ...
            <div className="mt-4 grid gap-3">
              ...
              >
                ...
                >
                  <path
                    fill="currentColor"
                    d="M21.35 11.1h-9.18v2.98h5.31c-.23 1.38-1.6 4.05-5.31 4.05c-3.2 0-5.81-2.64-5.81-5.89s2.61-5.89 5.81-5.89c1.83 0 3.06.77 3.76 1.43l2.57-2.49C16.86 3.46 15 2.5 12.17 2.5C6.98 2.5 2.73 6.74 2.73 11.99S6.98 21.5 12.17 21.5c6.19 0 10.26-4.34 10.26-10.44c0-.7-.08-1.16-.18-1.96"
                  />
                </svg>
                Kontynuuj z Google
              </Button>
          ...
          {!isRecovery ? (
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        <span className="inline-flex items-center gap-1">
                          <Lock className="size-4" aria-hidden /> Hasło
                        </span>
                      </FormLabel>
                      <FormControl>
                        <Input
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          <span className="inline-flex items-center gap-1">
                            <KeyRound className="size-4" aria-hidden /> Kod 2FA
                          </span>
                        </FormLabel>
                        <FormControl>
                          <Input
            ...
            <form
              ...
              className="grid gap-3"
              aria-label="Formularz resetu hasła"
            >
              <div>
              ...
              <p className="text-xs text-muted-foreground">
                Jeśli ten link wygasł, poproś o nowy w formularzu logowania.
              </p>
            </form>
          )}
        </CardContent>
